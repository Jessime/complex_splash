...
shield_touch = False
score = False
while current_time < end_time and not score:
    
    #Check for balloon/mouse movement
    loc = win.checkMouse()
    if loc and not shield_touch and not shot:
    ...
    #check for shot or exit
    last_key = win.checkKey()
    if last_key == 'space':
        shot = True
        
    #Move balloon
    if shot and not shield_touch and random.randint(0, 20) == 1:
        ball.move(1, 0)
        
    #check for balloon hitting a shield
    for s in shields:
